...
########################################################################################################################
def get_new_mails():
    ret, tmp_mails = init.connection.search(None, 'UNSEEN')
...
def create_question_from_mail(mail):
    ...
    :rtype : init.Question
    """
    ret, data = init.connection.fetch(mail, '(INTERNALDATE BODY[HEADER.FIELDS (MESSAGE-ID FROM)])')
    ...
    return init.Question(id=id, sender=sender, is_answered=is_answered, sent_on=sent_on,
                         answered_by=answered_by, answered_on=answered_on)


########################################################################################################################
def statistics():
    unanswered = session.execute()
...
def main():
    ...
    for mail in mails:
        ...
        ret, data = init.connection.fetch(mail, 'BODY[HEADER.FIELDS (FROM IN-REPLY-TO)]')
        ...
        init.connection.uid('store', mail, '+FLAGS', '\Seen')
